...
import com.fate.api.customer.handler.wx.applet.WaMemberCardHandler;
...
/**
 * @program: parent
 * @description: 商户自己的微信公众号功能配置（此项目主要是处理回调）
 * @author: chenyixin
...
public class WxMpConfiguration {
    ...
    @Autowired
    private  WaMemberCardHandler waMemberCardHandler;
    @Autowired
    ...
    public WxMpMessageRouter newRouter(WxMpService service) {
        ...
        newRouter.rule().async(false).msgType(WxConsts.XmlMsgType.EVENT)
                .event(WxConsts.EventType.CARD_SUBMIT_MEMBERCARD_USER_INFO)
                .handler(waMemberCardHandler).end();
        return newRouter;
    }
