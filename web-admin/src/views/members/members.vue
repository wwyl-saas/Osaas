<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.query" placeholder="输入手机号查找" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    <el-dialog :visible.sync="dialogVisible" width="12%">
    ...
    >
      ...
      <el-table-column label="用户头像" width="120" align="center">
        <template slot-scope="{row}">
          <img :src="row.avatar" style="max-height:50px" @click="handlePictureClick(row)">
        </template>
      </el-table-column>
      <el-table-column label="姓名" width="150" header-align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="昵称" width="150" header-align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="会员级别" width="150" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="积分" width="150" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.memberLevelCode }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchMemberList } from '@/api/members'
...
export default {
  ...
  methods: {
    ...
    handleFilter() {
      ...
      if (this.listQuery.query === '') this.listQuery.mobile = undefined
      this.getList()
    },
