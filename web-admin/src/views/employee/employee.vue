<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.mobile" placeholder="输入手机号查找" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="用户头像" width="120" align="center">
        <template slot-scope="{row}">
          <img :src="row.profileUrl" style="max-height:50px" @click="handlePictureClick(row)">
        </template>
      </el-table-column>
      <el-table-column label="用户名" width="150" header-align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="艺名" width="150" header-align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="职称" width="120" header-align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.postTitle }}</span>
        </template>
      </el-table-column>
      <el-table-column label="用户简介" width="250" header-align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.userBrief }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否预约" align="center" width="80">
        <template slot-scope="scope">
          <span v-if="scope.row.ifAppointment">是</span>
          <span v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column label="服务评分" align="center" width="200">
        <template slot-scope="scope">
          <el-rate
