package com.fate.api.merchant.endpoint;

import com.alibaba.fastjson.JSON;
import com.fate.api.merchant.common.SpringContext;
 ...
 **/
@ServerEndpoint(value = "/websocket/settle")
@Component
@Slf4j
public class SettleWebSocket {
    ...
    public void onOpen(Session session) throws IOException {
        try {
            ...
            JwtToken jwtToken = jwtService.verifierToken(token.orElseThrow(() -> new BaseException(ResponseInfo.INVALID_PARAM))).orElseThrow(() -> new AuthException());
            CacheService cacheService = SpringContext.getBean(CacheService.class);
            String existIp = (String) cacheService.get(RedisKey.MERCHANT_SOCKET_KEY + jwtToken.getUserId());
            if (StringUtils.isBlank(existIp)) {
                String netIp = getIp();
                cacheService.set(RedisKey.MERCHANT_SOCKET_KEY + jwtToken.getUserId(), netIp, 90L);//两分钟持续时间，需要不断刷新,nginx限制为90秒
                UserSessionUtil.add(jwtToken.getUserId(), session);
            ...
            session.close(new CloseReason(CloseReason.CloseCodes.UNEXPECTED_CONDITION, "权限校验失败"));
        }
    }
    ...
    public void onClose(Session session) {
        log.info("前端主动关闭连接");
        CacheService cacheService = SpringContext.getBean(CacheService.class);
        Long customerId = UserSessionUtil.getUserId(session).orElseThrow(() -> new BaseException(ResponseInfo.SESSION_INFO_ERROR));
        cacheService.del(RedisKey.MERCHANT_SOCKET_KEY + customerId);
        UserSessionUtil.remove(session);
    }

    /**
     * 收到客户端消息后调用的方法  只支持修改优惠券返回新的订单金额
     *
     * @param message
     * @param session
     * @throws IOException
    ...
    public void onMessage(String message, Session session) throws IOException {
        ...
        try {
            Long customerId = UserSessionUtil.getUserId(session).orElseThrow(() -> new BaseException(ResponseInfo.SESSION_INFO_ERROR));
            cacheService.expire(RedisKey.MERCHANT_SOCKET_KEY + customerId, 90L);//两分钟持续时间，需要不断刷新,nginx限制为90秒
    ...
    public void onError(Session session, Throwable error) {
        Long userId = UserSessionUtil.getUserId(session).orElseThrow(() -> new BaseException(ResponseInfo.SESSION_INFO_ERROR));
        log.error("socket发生错误：{}，customerId： {}", error, userId);
    }
