  ...
  methods: {
    ...
    createDays: function(year, month) {
      ...
      for (let i = 1; i <= days; i++) {
        ...
        let dateString = year + '-' + date.monthFormat + '-' + date.dateFormat
        date.dateString = dateString
        let index = this.data.appointments.findIndex(item => item === dateString)
        if (index != -1) {
          date.appointment = true
        }
